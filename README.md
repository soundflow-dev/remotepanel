...
`TRANSFER_FILE_STREAMS` controls how many streams RemotePanel may use for one large non-SMB file. SMB transfers default to a single destination writer because many SMB servers can stall when several write handles target different offsets of the same large file. `TRANSFER_SMB_PARALLEL_FILES` controls how many separate SMB files can be copied at the same time and defaults to `8`, which can improve throughput when a transfer contains multiple large files while keeping each file write sequential and predictable. Non-SMB Turbo transfers can still copy different files in parallel.
