...
`TRANSFER_FILE_STREAMS` controls how many streams RemotePanel may use for one large non-SMB file. SMB transfers default to a single destination writer because many SMB servers can stall when several write handles target different offsets of the same large file. SMB transfers still use resumable blocks, but avoid copying multiple SMB files in parallel so writes remain predictable. Non-SMB Turbo transfers can still copy different files in parallel.
