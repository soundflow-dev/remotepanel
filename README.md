...
`TRANSFER_FILE_STREAMS` controls how many streams RemotePanel may use for one large file. `TRANSFER_SMB_FILE_STREAMS` caps that value whenever the source or destination is SMB. It defaults to `8` for aggressive high-speed homelab transfers. If a NAS, switch, or server becomes unstable during multi-TB transfers, lower this value first. Turbo can still transfer different files in parallel.
